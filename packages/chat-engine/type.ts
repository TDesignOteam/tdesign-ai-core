...
/**
 * 传输方式类型
 * - 'fetch': 非流式，普通 HTTP 请求（对应旧 stream: false）
 * - 'sse': SSE 流式传输（对应旧 stream: true，默认值）
 * - 'ws': WebSocket 流式传输
 */
export type ChatTransport = 'fetch' | 'sse' | 'ws';
...
export interface ChatNetworkConfig {
  ...
  /**
   * 是否启用流式传输
   * @deprecated 请使用 transport 代替。stream: true → transport: 'sse'，stream: false → transport: 'fetch'
   */
  stream?: boolean;
  /** WebSocket 专属配置（仅 transport: 'ws' 时生效） */
  ws?: ChatWSConfig;
